odoo.define('ob_pos_multi_uom.multi_uom', function (require) {
    ...
    models.Order = models.Order.extend({
        set_orderline_options: function(orderline, options) {
            var result = _super_order.set_orderline_options.call(this, orderline, options);
            if (options.uom_id) {
                orderline.uom_id = options.uom_id;
            }
            return result;
        },
    });
    ...
    models.Orderline = models.Orderline.extend({
        initialize: function(attr, options) {
            ...
            if (this.pos.config.is_multi_uom_shown && options.json && options.json.uom_id) {
                if (!isNaN(options.json.uom_id)) {
                    this.uom_id = this.pos.units_by_id[options.json.uom_id];
                } else {
                    this.uom_id = this.pos.units_by_id[options.json.uom_id.id];
                }
            } else {
